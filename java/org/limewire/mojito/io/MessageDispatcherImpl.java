/*
 * Mojito Distributed Hash Table (Mojito DHT) Copyright (C) 2006-2007 LimeWire LLC
 *
...
public class MessageDispatcherImpl extends MessageDispatcher implements Runnable {
  ...
  /**
   * A flag whether or not this MessageDispatcher is accepting incoming Requests and Responses.
   */
  ...
  /**
   * Whether or not a new ByteBuffer should be allocated for every message we receive.
   */
  ...
  /**
   * Returns whether or not a new ByteBuffer is allocated for every message.
   */
  ...
  /**
   * Writes all Messages (if possible) from the output queue to the Network and returns whether or
   * not some Messages were left in the output queue.
   */
  ...
  /**
   * Reads and returns a single DHTMessage from Network or null if no Messages were in the input
   * queue.
   */
  ...
  /**
   * Called by verify(SecureMessage, SecureMessageCallback) to execute the Runnable that does the
   * actual verification. You may override this method to execute the Runnable on a different
   * Thread.
   */
  ...
  /**
   * Called to indicate an interest in reading something from the Network. Override this method if
  ...
  /**
   * Called to indicate an interest in writing something to the Network. Override this method if you
  ...
  /**
   * The actual send method. Returns true if the data was sent or false if there was insufficient
   * space in the output buffer (that means you'll have to re-try it later again).
   * <p>
   * IMPORTANT: The expected behavior is the same as DatagramChannel.send(BytBuffer,SocketAddress).
   * That means if you are not able to send the data return false and leave the ByteBuffer
   * untouched!
   */
  ...
  private boolean send(SocketAddress dst, ByteBuffer data) throws IOException {
    ...
    InetSocketAddress resolved = new InetSocketAddress(ip.getHostName(), ip.getPort());
