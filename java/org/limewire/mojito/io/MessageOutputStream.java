...
public class MessageOutputStream extends DataOutputStream {
     ...
     */
    private void writeInetAddress(InetAddress addr) throws IOException {
     ...
     * Writes the given InetAddress to the OutputStream.
     */
    private void writeInetHostname(InetSocketAddress ip) throws IOException {
        byte[] address = ip.getHostName().getBytes();
        writeByte(address.length);
        write(address, 0, address.length);
    }
