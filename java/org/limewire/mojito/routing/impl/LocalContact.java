...
public class LocalContact implements Contact {
    ...
    private void init() {
        contactAddress = InetSocketAddress.createUnresolved("localhost", 0);
    }
    ...
    public synchronized void setExternalPort(int port) {
        ...
        setContactAddress(InetSocketAddress.createUnresolved(addr.getHostName(), port));
    }
    ...
    public synchronized boolean setExternalAddress(SocketAddress externalSocketAddress) {
        ...
        String externalHostname = ((InetSocketAddress)externalSocketAddress).getHostName();
        int externalPort = ((InetSocketAddress)externalSocketAddress).getPort();
        
        String currentHostname = ((InetSocketAddress)getContactAddress()).getHostName();
        
        if (externalHostname.equals(currentHostname)) {
            if (LOG.isInfoEnabled()) {
                LOG.info("Reported external address is equal to current external address: " + externalHostname);
            }
            return false;
        }
        ...
        SocketAddress addr = InetSocketAddress.createUnresolved(
